...
spec = describe "Glue.Lib.List.Reverse (Test reverse function)" do
    it "reverses a list" do
        ...
        result <- runEvalSimple (apply Reverse.reverse args) []
        case result of
            Left err -> expectationFailure $ "Reverse failed: " <> show err
            Right (res, _) -> res `shouldBe` List [Integer 3, Integer 2, Integer 1]
    ...
    it "reverses a single element is a list" do
        ...
        case result of
            Left err -> expectationFailure $ "Reverse failed: " <> show err
            Right (res, _) -> res `shouldBe` List [List [String "hello"]]

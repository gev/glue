...
spec = describe "Glue.Lib.Bool.Not (Test not function)" do
    describe "Logical negation" do
        it "returns false for true" do
            let args = [Bool True]
            result <- runEvalSimple (not_ args) (E.fromFrame lib)
            case result of
        ...
        it "! alias works identically to not" do
            ...
            result1 <- runEvalSimple (not_ args1) (E.fromFrame lib)
            result2 <- runEvalSimple (not_ args2) (E.fromFrame lib)
            result3 <- runEvalSimple (not_ args3) (E.fromFrame lib)

...
-- Extract the function from NativeFunc constructor for testing
notImpl :: [IR Eval] -> Eval (IR Eval)
notImpl args = case not_ of
    NativeFunc f -> f args
    _ -> error "not_ should be NativeFunc"
...
spec = describe "Glue.Lib.Bool.Not (Test not function)" do
    describe "Logical negation" do
        it "returns false for true" do
            let args = [Bool True]
            result <- runEvalSimple (notImpl args) []
            case result of
        ...
        it "! alias works identically to not" do
            ...
            result1 <- runEvalSimple (notImpl args1) []
            result2 <- runEvalSimple (notImpl args2) []
            result3 <- runEvalSimple (notImpl args3) []

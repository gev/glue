...
spec = describe "Glue.Lib.Bool.Not (Test not function)" do
    describe "Logical negation" do
        it "returns false for true" do
            let args = [Bool True]
            result <- runEvalSimple (apply not_ args) []
            case result of
        ...
        it "! alias works identically to not" do
            ...
            result1 <- runEvalSimple (apply not_ args1) []
            result2 <- runEvalSimple (apply not_ args2) []
            result3 <- runEvalSimple (apply not_ args3) []

...
spec = describe "Glue.Lib.Bool.Le (Test le function)" do
    describe "Less than or equal comparison" do
        it "returns true for equal numbers" do
            ...
            result <- runEvalSimple (le args) (E.fromFrame lib)
            case result of
        ...
        it "<= alias works identically to le" do
            ...
            result1 <- runEvalSimple (le args1) (E.fromFrame lib)
            result2 <- runEvalSimple (le args2) (E.fromFrame lib)
            result3 <- runEvalSimple (le args3) (E.fromFrame lib)

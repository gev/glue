module Reactor.Lib.Arithmetic.DivSpec (spec) where
...
import qualified Reactor.Lib.Arithmetic.Div as Div
import Test.Hspec
    ...
    describe "Div function" do
        it "returns 0.5 for (/ 2)" do
            ...
            case result of
                ...
                Right (res, _) -> res `shouldBe` Number (fromFloatDigits 0.5)
        ...
        it "returns 2.5 for (/ 10 2 2)" do
            ...
            case result of
                Left err -> expectationFailure $ "Div failed: " <> show err
                Right (res, _) -> res `shouldBe` Number (fromFloatDigits 2.5)

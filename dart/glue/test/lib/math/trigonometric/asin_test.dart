...
void main() {
  group('Glue.Lib.Math.Trigonometric.Asin (asin)', () {
    test('asin with common angles', () async {
    ...
    test('asin with integer input', () async {
      final result = await runCode('(asin 1)');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('asin with float input', () async {
      final result = await runCode('(asin 1.0)');
      result.match(
        (error) => fail('Should not be left: $error'),
        (value) => expect(value, isA<IrFloat>()),
      );
    ...
    test('fails with wrong argument types', () async {
      final result = await runCode('(asin "hello")');
      expect(result.isLeft, isTrue);
    });

    test('fails with too few arguments', () async {
      final result = await runCode('(asin)');
      expect(result.isLeft, isTrue);
    });

    test('fails with too many arguments', () async {
      final result = await runCode('(asin 1 2)');
      expect(result.isLeft, isTrue);
    });
  });
}

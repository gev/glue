...
void main() {
  group('Glue.Lib.Math.Trigonometric.Asin (asin)', () {
    test('asin with common angles', () {
      final result = runCode('(asin 0)');
      result.match(
    ...
    test('asin with integer input', () {
      final result = runCode('(asin 1)');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('asin with float input', () {
      final result = runCode('(asin 1.0)');
      result.match(
        (error) => fail('Should not be left: $error'),
        (value) => expect(value, isA<IrFloat>()),
      );
    ...
    test('fails with wrong argument types', () {
      final result = runCode('(asin "hello")');

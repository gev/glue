...
void main() {
  group('Glue.Lib.Math.Utility.Abs (abs)', () {
    test('abs with positive integer returns same integer', () {
    ...
    test('abs with negative integer returns positive', () {
      final result = runCode('(abs -5)');
      result.match(
        (error) => fail('Should not be left: $error'),
        (value) => expect(value, equals(IrInteger(5))),
    ...
    test('abs with positive float returns same float', () {
      final result = runCode('(abs 3.5)');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('abs with negative float returns positive', () {
      final result = runCode('(abs -3.5)');
      result.match(
        (error) => fail('Should not be left: $error'),
        (value) => expect((value as IrFloat).value, closeTo(3.5, 0.0001)),
    ...
    test('fails with wrong argument types', () {
      final result = runCode('(abs "hello")');

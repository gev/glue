...
void main() {
  group('Glue.Lib.Math.Utility.Floor (floor)', () {
    test('floor with positive float returns integer', () {
      final result = runCode('(floor 3.7)');
      result.match(
    ...
    test('floor with negative float returns integer', () {
      final result = runCode('(floor -3.1)');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('floor with integer input return same integer', () {
      final result = runCode('(floor 5)');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('fails with wrong argument types', () {
      final result = runCode('(floor "hello")');

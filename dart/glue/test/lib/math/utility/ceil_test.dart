...
void main() {
  group('Glue.Lib.Math.Utility.Ceil (ceil)', () {
    test('ceil with positive float returns integer', () {
      final result = runCode('(ceil 3.7)');
      result.match(
    ...
    test('ceil with negative float returns integer', () {
      final result = runCode('(ceil -3.7)');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('ceil with integer input return same integer', () {
      final result = runCode('(ceil 5)');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('fails with wrong argument types', () {
      final result = runCode('(ceil "hello")');

...
void main() {
  group('Glue.Lib.List.Take (take)', () {
    test('takes first N elements from list', () async {
      final result = await runCode('(take 3 (1 2 3 4 5))');
      result.match(
    ...
    test('takes fewer elements when N > list length', () async {
      final result = await runCode('(take 10 (1 2 3))');
      result.match(
        (error) => fail('Should not be left: $error'),
        (value) => expect(
          value,
    ...
    test('takes zero elements', () async {
      final result = await runCode('(take 0 (1 2 3))');
      result.match(
        (error) => fail('Should not be left: $error'),
    ...
    test('takes all elements when N equals list length', () async {
      final result = await runCode('(take 3 (1 2 3))');
      result.match(
        (error) => fail('Should not be left: $error'),
        (value) => expect(
          value,
    ...
    test('fails on negative count', () async {
      final result = await runCode('(take -1 (1 2 3))');
    ...
    test('fails on non-list second argument', () async {
      final result = await runCode('(take 3 42)');
      expect(result.isLeft, isTrue);
    });

    test('fails with wrong number of arguments', () async {
      final result = await runCode('(take 3)');
      expect(result.isLeft, isTrue);
    });

    test('fails with too many arguments', () async {
      final result = await runCode('(take 3 (1 2 3) (4 5 6))');
      expect(result.isLeft, isTrue);
    });
  });
}

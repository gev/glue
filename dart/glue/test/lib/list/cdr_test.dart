...
void main() {
  group('Glue.Lib.List.Cdr (cdr)', () {
    test('returns the rest of a list', () async {
      final result = await runCode('(cdr (1 2 3))');
      result.match(
        (error) => fail('Should not be left: $error'),
        (value) => expect(value, equals(IrList([IrInteger(2), IrInteger(3)]))),
      );
    });

    test('fails on empty list', () async {
      final result = await runCode('(cdr ())');
      expect(result.isLeft, isTrue);
    });

    test('fails on non-list', () async {
      final result = await runCode('(cdr 42)');
      expect(result.isLeft, isTrue);
    });
    ...
    test('fails with too many arguments', () async {
      final result = await runCode('(cdr (1 2 3) (4 5 6))');
      expect(result.isLeft, isTrue);
    });
  });
}
